...
async def fetch(session, url):
    # Usamos ssl=False para omitir la verificación del certificado
...
async def main():
    # Usamos tqdm para crear una barra de progreso con las tareas a ejecutar
    tasks = [procesarProvincia(p) for p in provincias]

    # Asignamos un loop para que la barra de progreso se actualice con el avance
    with tqdm(total=len(tasks), desc="Procesando provincias", unit="provincia") as pbar:
        results = []
        # Recorremos las tareas y actualizamos la barra de progreso
        for result in await asyncio.gather(*tasks):
            results.append(result)
            pbar.update(1)  # Actualizamos la barra de progreso

    # Actualizamos el diccionario con los resultados
    for result in results:
        distritos_dict.update(result)

    # Guardamos los resultados en un archivo JSON
    with open('json/distritos.json', 'w', encoding='utf-8') as f:
        json.dump(distritos_dict, f, indent=4, ensure_ascii=False)

    print(f"Datos guardados en: distritos.json.")
