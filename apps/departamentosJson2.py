...
def procesarURL(pliego, gasto, ap):
    ...
    try:
        ...
        if not htmlTabla:
            logError(f"No se encontró la tabla para el pliego {pliego}(gasto {gasto}, {ap}): {url}")
            return

        htmlTabla2 = htmlTabla.findAll('td')
        if not htmlTabla2:
            logError(f"No se encontró el td para el pliego {pliego}(gasto {gasto}, {ap}): {url}")
            return
        ...
        logError(f"Error procesando {pliego}, {gasto}, {ap}: {e}, {url}")
        return []
