...
def procesarURL(ubigeo, gasto, ap):
    ...
    try:
        ...
        if not htmlTabla:
            logError(f"No se encontró la tabla para el ubigeo {ubigeo}, gasto {gasto}, ap {ap}: {url}")
            return

        htmlTabla2 = htmlTabla.findAll('td')
        if not htmlTabla2:
            logError(f"No se encontró el td para el ubigeo {ubigeo}, gasto {gasto}, ap {ap}: {url}")
            return
        ...
        logError(f"Error procesando {ubigeo}, {gasto}, {ap}: {e}, {url}")
        return []
