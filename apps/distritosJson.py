...
def procesarURL(ubigeo, gasto, ap, distrito):
    ...
    if ap == '':
        url = (f'{URL_MEF}?_uhc=yes&0=&1=M&37=M&5={departamento}&6={provincia}&7={distrito}&{gasto}&y={anio}&cpage=1&psize=400&of=col2&od=1')
    else:
        url = (f'{URL_MEF}?_uhc=yes&0=&1=M&37=M&5={departamento}&6={provincia}&7={distrito}&{gasto}&y={anio}&ap={ap}&cpage=1&psize=400&of=col2&od=1')
...
def procesarDistrito(ubigeo, enQueGasta, actividadProyectos):
    ...
    if distrito:
        ...
        for gasto in enQueGasta:
            for ap in actividadProyectos:
                filas = procesarURL(ubigeo, gasto, ap, distrito)  # Obtener filas para la combinación
...
def main():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=12) as executor:
            futures = {
                executor.submit(procesarDistrito, ubigeo, enQueGasta, actividadProyectos): ubigeo
                for ubigeo in distritos
            }

            # Monitorear el progreso de las tareas
            for future in tqdm(as_completed(futures), total=len(distritos), desc="Procesando distritos"):
                ubigeo = futures[future]
                try:
                    future.result()  # Verificar el resultado de la tarea
                except Exception as e:
                    logError(f"Error general procesando distritos {ubigeo}: {e}")
