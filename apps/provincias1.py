...
def procesarURL(ubigeo, gasto, ap, a, conexion):
	...
	for td in htmlTabla2:
		...
		if i == 1:
			...
			if len(montos) == 0:
				filaActual.append(0)
			else: filaActual.append(montos)
		elif i == 9:
			avance= valor.strip()
			if len(avance) == 0:
				filaActual.append(0)
			else: filaActual.append(avance)
		else: filaActual.append(valor)
...
def main():
	...
	try:
		...
		with ThreadPoolExecutor(max_workers=1) as executor:
			futures = [
				executor.submit(procesarURL, ubigeo, gasto, ap, a, conexion)
